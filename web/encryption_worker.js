/*
 * Web worker to hash password with argon, depends on subset of hash-wasm
 * lib which is vendored in this file.
 *
...
async function hashPassword({ password, salt }) {
  return await hashwasm.argon2id({
    password: new TextEncoder().encode(password), // utf-8,
    salt: salt,
    iterations: 6,
...
async function verifyPassword({ password, hash }) {
  const isValid = await hashwasm.argon2Verify({ password, hash });
  return isValid.toString();
}

self.onmessage = async function (e) {
  const { id, action, data: data_ } = JSON.parse(e.data);
  ...
  try {
    let result;
    if (action === "hash") {
      result = await hashPassword(data);
    } else if (action === "verify") {
      result = await verifyPassword(data);
    } else {
...
(function (global, factory) {
  ...
  ***************************************************************************** */
   ...
   * @returns Computed hash */ function argon2i(options) {
    return __awaiter(this, void 0, void 0, function* () {
